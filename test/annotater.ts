...
export function createPropertiesAnnotater(indentation: string, callback: (str: string) => void): sp.PropertySubscribers {
    return {
        array: (key, ac, keyRange) => {
            ...
            ac.onElement(createValuesAnnotater(`${indentation}\t`, callback))
            ac.onEnd(end => {
...
export function createValuesAnnotater(indentation: string, callback: (str: string) => void): sp.ValueSubscribers {
    return {
        array: (ac) => {
            console.log(`${indentation}[ //`, printLoc(ac.start))
            ac.onElement(createValuesAnnotater(`${indentation}\t`, callback))
            ac.onEnd(end => {
                console.log(`${indentation}] //`, printLoc(end))
            })
        },
        object: (oc) => {
        ...
        value: (value, range) => {
            console.log(`${indentation}${format(value)} //`, printRange(range))
        },
    }
}