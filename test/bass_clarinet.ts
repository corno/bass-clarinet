...
type EventDefinition = [string, string | undefined | false | true | null | number]

function generic(key: any, prechunked: boolean, sep?: any) {
  return function () {
    const doc = docs[key].text
    const events = docs[key].events
    const l: any[] = []
    const doc_chunks: string[] = !prechunked ? doc.split(sep) : docs[key].chunks!
