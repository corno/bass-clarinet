...
function assert1(expr: boolean, msg?: string) {
    assert.ok(expr, msg)
}
...
type EventDefinition = [string, string | undefined | false | true | null | number, number?, number?]
        ...
        EVENTS.forEach(function (event) {
            function x(value: any) {
                ...
                if (temp_env.CRECORD) { // for really big json we dont want to type all
                    ...
                    assert1(currentExpectedEvent[0] === event, 'event: ' + currentExpectedEventIndex + ', expected type: [' + currentExpectedEvent[0] + '] got: [' + event + ']');
                    if (event !== 'error') {
                        assert1(currentExpectedEvent[1] === value, 'event:' + currentExpectedEventIndex + ' expected value: [' + currentExpectedEvent[1] + '] got: [' + value + ']');
                    } else {
